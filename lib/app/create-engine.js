...
const { isEmptyPath } = require('../utils')
...
class Engine {
  ...
  async createEnhanceApp () {
    await fs.outputFile(
      ...
      isEmptyPath(this.lib.resolveDvanDir('enhanceApp.js'))
    ...
    return formatCode(`
      ...
      export function createApp () {
        ...
        const app = new Vue({
          ...
          components: {
            SWUpdatePopup: () =>
              import('${path.resolve(__dirname, '../pwa/sw-update-popup.vue')}')
          },
          render: h => h(
            'div',
            {
        ...
        },
        {
          ...
          component: () => {
            return import(/* webpackChunkName: "404" */
              '${
                isEmptyPath(this.lib.resolveBaseDir('pages', '404.vue'))
