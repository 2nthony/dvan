...
class Dvan {
  ...
  async prepare ({ dev }) {
    ...
    this.publicPath = dev ? '/' : this.config.root
    ...
    if (!dev) {
      this.serverConfig = require('./webpack/server')(this)
    }
    ...
    this.PWAPlugin = require('./pwa')(this)
  ...
  async build () {
    ...
    function handlePath (route) {
      ...
      return /index|404/.test(route) ? `${route}.html` : `${route}/index.html`
    }
    ...
    async function renderPage (route, { title, description, head }) {
      ...
      await console.log(
        `> Generated file ${
          path.relative(process.cwd(), filePath(handlePath(route)))
        }`
      )
    }
    ...
    // ResolvePWA hook
    await this.PWAPlugin.resolvePWA()
