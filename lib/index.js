...
class Dvan {
  ...
  async prepare ({ dev }) {
    ...
    if (!dev) {
      await fs.emptyDir(this.resolveBaseDir(this.config.outputDir))
    }
  }
  ...
  async build () {
    ...
    const outputDir = this.resolveBaseDir(this.config.outputDir)
    ...
    await this.compile(this.clientConfig.toConfig())
  }
  ...
  async ssg () {
    ...
    const outputDir = this.resolveDvanDir()
    ...
    this.serverConfig
      .entry('app')
      .add(`${this.engine.resolveEngineDir('server-entry')}`)
      .end()
      .output
      .path(outputDir)
      .end()
    ...
    const filePath = filename => this.resolveBaseDir(this.config.outputDir, filename)
    routes.map(async route => {
      ...
      const context = {
        ...
        renderHead: () => renderHead(this.config.head)
      }
    ...
    // Move assets
    await fs.move(
      path.join(outputDir, this.config.assetsDir),
      this.resolveBaseDir(this.config.outputDir, this.config.assetsDir)
    )

    // Remove manifest
    await fs.remove(path.join(outputDir, 'manifest'))
  }
}
